...
/**
 * this class implements ActionKeyword and performs method that select the option from a dropdown through its text(NUMBERS only) options
 * @author edward.cervantes
 */
public class SelectFromDropdownByVisibleNumbersOnlyKeyword implements ActionKeyword {

    private static final Logger log = Logger.getLogger(SelectFromDropdownByVisibleNumbersOnlyKeyword.class);

    //	public static final LoggerAdapter log = CustomLogFactory.getLogger(SelectFromDropdownByVisibleNumbersOnlyKeyword.class);
    ...
    public boolean isPerform(String pageObjectId, String dataInput, WebDriver driver) throws Exception {
        ...
        try {
            if (log.isDebugEnabled()) {
                log.debug("MCI >> selectFromDropdownByVisibleNumbersOnly()");
            }
            log.info("Selecting from " + pageObjectId);

            //			System.out.println(dataInput + " " + pageObjectId + "......");
        ...
        if (log.isDebugEnabled()) {
            log.debug("MCO << selectFromDropdownByVisibleNumbersOnly()");
        }
        return isWorking;
    }
