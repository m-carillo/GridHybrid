 ...
 */
public class IsAscendingKeyword implements ActionKeyword {
    ...
    public boolean isPerform(String pageObjectId, String dataInput, WebDriver driver) throws Exception {
        ...
        try {
            if (log.isDebugEnabled()) {
                log.debug("MCI >> isAscending()");
            }
            log.info("Checking if objects are in 'ascending' order...");
            ...
            for (int i = 1; i < retrievedValueList.size(); i++) {
                if (retrievedValueList.get(i - 1).compareTo(retrievedValueList.get(i)) > 0) {
                	isWorking = false;
                    reasonMessage = "Values are not arranged in ascending format...";
                    break;
                }
            }
