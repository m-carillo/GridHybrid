 ...
 * containing text values of the input
 * input format(count, value to compare)
 * input format(value to compare, count)
 * @author edward.cervantes
 */
public class CountContainsTextByCssSelectorKeyword implements ActionKeyword {
    ...
    public boolean isPerform(String pageObjectId, String dataInput, WebDriver driver) throws Exception {
        ...
        try {
            ...
            log.info("Comparing the number of elements that contains the input text value to that of the input number value...");
            ...
            if (!(dataInput.contains(",") || dataInput.contains(";"))) {
                ...
                reasonMessage = new StringBuilder("expected: ").append(dataCount)
                        .append(" retrieved: ")
                        .append(elemSize)
                        .toString();
                driverScript.setReason(reasonMessage);
