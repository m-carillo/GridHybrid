...
/**
 * this class implements ActionKeyword and performs method that checks if the element is enabled
 * @author edward.cervantes
 */
public class IsElementEnabledKeyword implements ActionKeyword {

    private static final Logger log = Logger.getLogger(IsElementEnabledKeyword.class);
    ...
    public boolean isPerform(String pageObjectId, String dataInput, WebDriver driver) throws Exception {
        ...
        try {
            if (log.isDebugEnabled()) {
                log.debug("MCI >> isElementEnabled()");
            }
            log.info("Checking if WebElement is enabled: " + pageObjectId);
            ...
            isWorking = false;
            reasonMessage = new StringBuilder("Not able to check if element is enabled due to ").append(e.getMessage())
                    .append("...")
                    .toString();
        ...
        if (log.isDebugEnabled()) {
            log.debug("MCO << isElementEnabled()");
        }
        return isWorking;
    }
